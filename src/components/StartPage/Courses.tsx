  ...
  export const Courses: React.FC = () => {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <FetchUserCoursesButton />
      </div>
    );
  };

  const FetchUserCoursesButton: React.FC = () => {
    const fetchUserCourses = async () => {
      const userId = localStorage.getItem('currentID');
      if (!userId) {
        console.error('currentID не найден в localStorage');
        return;
      }

      try {
        const response = await axiosInstance.get(`/courses/user/${userId}`);
        console.log(`Успешный GET-запрос для пользователя ${userId}:`, response.data);
      } catch (error) {
        console.error(`Ошибка при выполнении GET-запроса для пользователя ${userId}:`, error);
      }
    };
