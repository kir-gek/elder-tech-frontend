...
export const CourseDetail: React.FC = () => {
  ...
  const handleJoinCourse = async () => {
    try {
      ...
      console.error(
        "Ошибка при выполнении POST-запроса для записи на курс:",
        error
      );
    }
  };
  ...
  if (!course) {
    return (
      <div className="text-center py-8 text-red-500 font-bold text-xl">
        Курс не найден
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <p className="text-gray-700 mb-4">
          <strong>О курсе:</strong> {course.about}
        </p>
        <p className="text-gray-700 mb-4">
          <strong>Для кого:</strong> {course.for_who}
        </p>
        <p className="text-gray-700 mb-4">
          <strong>Требования:</strong> {course.requirements}
        </p>
        <p className="text-gray-700 mb-4">
          <strong>Категории:</strong> {course.categories.join(", ")}
        </p>
        <p className="text-gray-700 mb-4">
